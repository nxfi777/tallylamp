# Compile TypeScript with devDependencies present.
# NODE_ENV=production here would skip typescript, and `npx tsc` would then
# fetch the unrelated npm package `tsc@2.0.4`.
...
ENV HOST=0.0.0.0
ENV PORT=8080
# V8 sizes its heap from the host's visible memory, which on Railway is 32 GB: a large
# reserved old-space and lazy collection. A small semi-space is the single biggest idle
# win, and the old-space cap bounds the ceiling rather than the floor.
ENV NODE_OPTIONS="--max-semi-space-size=2 --max-old-space-size=256"
EXPOSE 8080
