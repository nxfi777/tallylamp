...
/**
 * The audit table is append-only and was never bounded — it grew the database by ~26 MB
 * over a burst of failed-auth attempts alone. Keep a generous tail rather than everything.
 */
export function pruneAuditEvents(keep = 50_000): number {
  const res = getDb()
    .prepare(`DELETE FROM audit_events WHERE id < (SELECT MAX(id) - ? FROM audit_events)`)
    .run(keep);
